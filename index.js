...
function findLargestInteger(integers) {
  ...
  // }
  // return result
  let maxNum = integers.reduce((max, int) => {
    return int.integer > max.integer ? int : max;
  });
  return maxNum;
}
...
class Car {
   ...
   */
  drive(distance) {
    const gallonsLeft = Math.floor(distance / this.mpg)
    this.tank = gallonsLeft
    if(this.odometer === 600) {
      return `${this.odometer += distance} (ran out of Gas!)`
    } else {
